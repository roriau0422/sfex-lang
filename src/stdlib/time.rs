            ...
            Arc::new(
                Box::new(|args| {
                    ...
                    let dt: DateTime<Local> = if args.is_empty() {
                        ...
                        Local.timestamp_opt(timestamp, 0).single().ok_or("Invalid timestamp")?
                    };
        ...
        Value::NativeFunction(
            Arc::new(
                Box::new(|args| {
                    if args.len() > 1 {
                        return Err(
                            "Time.GMTime requires 0-1 arguments (optional timestamp)".to_string()
                        );
                    }
                    ...
                    let dt: DateTime<Utc> = if args.is_empty() {
                        ...
                        Utc.timestamp_opt(timestamp, 0).single().ok_or("Invalid timestamp")?
                    };
        ...
        Value::NativeFunction(
            Arc::new(
                Box::new(|args| {
                    if args.len() != 2 {
                        return Err(
                            "Time.Format requires 2 arguments (datetime, format)".to_string()
                        );
                    }
...
fn create_datetime_map<Tz: TimeZone>(dt: DateTime<Tz>) -> Value where Tz::Offset: std::fmt::Display {
    let mut dt_map = HashMap::new();

    use bigdecimal::BigDecimal;

    dt_map.insert("Year".to_string(), Value::Number(BigDecimal::from(dt.year())));
    dt_map.insert("Month".to_string(), Value::Number(BigDecimal::from(dt.month() as i32)));
    dt_map.insert("Day".to_string(), Value::Number(BigDecimal::from(dt.day() as i32)));
    dt_map.insert("Hour".to_string(), Value::Number(BigDecimal::from(dt.hour() as i32)));
    dt_map.insert("Minute".to_string(), Value::Number(BigDecimal::from(dt.minute() as i32)));
    dt_map.insert("Second".to_string(), Value::Number(BigDecimal::from(dt.second() as i32)));

    let weekday = dt.weekday().number_from_monday();
    dt_map.insert("Weekday".to_string(), Value::Number(BigDecimal::from(weekday as i32)));

    dt_map.insert("YearDay".to_string(), Value::Number(BigDecimal::from(dt.ordinal() as i32)));

    dt_map.insert("Timestamp".to_string(), Value::Number(BigDecimal::from(dt.timestamp())));
