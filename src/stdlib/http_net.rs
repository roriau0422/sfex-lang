...
pub fn create_http_module(interpreter: &Interpreter) -> Value {
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            ...
            let result = runtime.block_on(async {
                ...
                if args.len() == 2 {
                    if let Value::Map(headers_map) = &args[1] {
                        for (key, value) in headers_map.read().expect("lock poisoned").iter() {
            ...
            match result {
                Ok(response) => {
                    let response_obj = runtime_get.block_on(create_response_object(response));
                    Ok(response_obj)
                }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            if args.is_empty() || args.len() > 3 {
                return Err(
                    "HTTP.Post requires 1-3 arguments (url, optional body, optional headers)"
                        .to_string(),
                );
            }
            ...
            match result {
                Ok(response) => {
                    let response_obj = runtime_post.block_on(create_response_object(response));
                    Ok(response_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            if args.is_empty() || args.len() > 3 {
                return Err(
                    "HTTP.Put requires 1-3 arguments (url, optional body, optional headers)"
                        .to_string(),
                );
            }
            ...
            match result {
                Ok(response) => {
                    let response_obj = runtime_put.block_on(create_response_object(response));
                    Ok(response_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            ...
            match result {
                Ok(response) => {
                    let response_obj = runtime_delete.block_on(create_response_object(response));
                    Ok(response_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            if args.is_empty() || args.len() > 3 {
                return Err(
                    "HTTP.Patch requires 1-3 arguments (url, optional body, optional headers)"
                        .to_string(),
                );
            }
            ...
            match result {
                Ok(response) => {
                    let response_obj = runtime_patch.block_on(create_response_object(response));
                    Ok(response_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            ...
            match result {
                Ok(response) => {
                    let stream_obj = runtime_getstream
                        .block_on(create_stream_object(response, runtime_getstream.clone()));
                    Ok(stream_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
    ...
    methods.insert(
        ...
        Value::NativeFunction(Arc::new(Box::new(move |args| {
            if args.is_empty() || args.len() > 3 {
                return Err(
                    "HTTP.PostStream requires 1-3 arguments (url, optional body, optional headers)"
                        .to_string(),
                );
            }
            ...
            match result {
                Ok(response) => {
                    let stream_obj = runtime_poststream
                        .block_on(create_stream_object(response, runtime_poststream.clone()));
                    Ok(stream_obj)
                }
                Err(e) => Err(format!("HTTP Error: {}", e)),
            }
