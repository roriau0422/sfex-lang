        ...
        Value::NativeFunction(Arc::new(Box::new(|args| {
            ...
            match std::fs::File::open(&filepath) {
                Ok(mut file) => {
                    ...
                    let priorities = guess_format_priority(&content_sample, Some(&filepath));

                    let (final_format, final_media_type) = if base_name == "Plain Text"
                ...
                match format {
                    ...
                    "TOML" => {
                        if let Ok(table) = ::toml::from_str::<::toml::Table>(&content) {
                            return Ok(toml::convert_toml_to_object(::toml::Value::Table(table)));
                        }
                    }
                    "XML" => {
        ...
        Value::NativeFunction(Arc::new(Box::new(|args| {
            ...
            match std::fs::File::open(&filepath) {
                Ok(mut file) => {
                    ...
                    description.insert("Format".to_string(), Value::String(final_format.clone()));
                    description.insert("MediaType".to_string(), Value::String(final_media_type));
                    description.insert(
                        "Extension".to_string(),
                        Value::String(format.extension().to_string()),
                    );
