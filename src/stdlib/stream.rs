...
pub fn create_stream_module() -> Value {
    ...
    methods.insert(
        ...
        Value::NativeFunction(
            Arc::new(Box::new(|_args| { Ok(create_stream_object(vec![], None)) }))
        )
    );
                            ...
                            Box::new(move |_args| {
                                ...
                                if *curr > end {
                                    ...
                                    *curr += 1;
                                    Ok(
                                        Value::Option(
                                            Box::new(Some(Value::Number(BigDecimal::from(val))))
                                        )
                                    )
                                }
                            })
...
pub fn create_stream_object(items: Vec<Value>, generator: Option<Value>) -> Value {
    let state = Arc::new(
        std::sync::RwLock::new(StreamState {
            items,
            index: 0,
            exhausted: false,
            generator,
...
fn create_filter_stream(parent_stream: Value, filter_fn: Value) -> Result<Value, String> {
                                ...
                                if let Value::NativeFunction(f) = next_method {
                                    match f(vec![]) {
                                        Ok(Value::Option(opt)) => {
                                            if let Some(item) = opt.as_ref() {
                                                if
                                                    let Value::NativeFunction(filter_f) =
                                                        &filter_fn_next
                                                {
                                                    match filter_f(vec![item.clone()]) {
                                                        ...
                                                        Ok(_) => {
                                                            return Err(
                                                                "Filter function must return Boolean".to_string()
                                                            );
                                                        }
                                                        Err(e) => {
                                                            return Err(e);
                                                    ...
                                                    return Err(
                                                        "Filter must be a function".to_string()
                                                    );
                                                }
                                            } else {
    ...
    add_close_method(&mut *stream_rc.write().expect("lock poisoned"), parent_stream.clone());
    add_transform_methods(&mut *stream_rc.write().expect("lock poisoned"), parent_stream.clone());
